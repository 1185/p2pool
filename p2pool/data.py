...
DONATION_SCRIPT = '76a9149abfb66a354edf11e9d2ce7135ff89e797d90c9e88ac'.decode('hex')
...
class NewShare(object):
    ...
    def generate_transaction(cls, tracker, share_data, block_target, desired_timestamp, desired_target, ref_merkle_link, desired_other_transaction_hashes_and_fees, net, known_txs=None, last_txout_nonce=0, base_subsidy=None):
        ...
        def get_share(header, last_txout_nonce=last_txout_nonce):
            ...
            share = cls(net, None, dict(
                ...
                hash_link=prefix_to_hash_link(starwels_data.tx_type.pack(gentx)[:-32-8-4], cls.gentx_before_refhash),
    ...
    def get_ref_hash(cls, net, share_info, ref_merkle_link):
        return pack.IntType(256).pack(starwels_data.check_merkle_link(starwels_data.hash256(cls.ref_type.pack(dict(
    ...
    def __init__(self, net, peer_addr, contents):
        ...
        merkle_root = starwels_data.check_merkle_link(self.gentx_hash, self.merkle_link)
    ...
    def check(self, tracker):
        ...
        if starwels_data.hash256(starwels_data.tx_type.pack(gentx)) != self.gentx_hash:
            raise ValueError('''gentx doesn't match hash_link''')
        
        if starwels_data.calculate_merkle_link([None] + other_tx_hashes, 0) != self.merkle_link:
...
def get_warnings(tracker, best_share, net, starwelsd_getinfo, starwelsd_work_value):
    ...
    if starwelsd_getinfo['errors'] != '':
        if 'This is a pre-release test build' not in starwelsd_getinfo['errors']:
            res.append('(from starwelsd) %s' % (starwelsd_getinfo['errors'],))
