...
from p2pool.starwels import data as starwels_data, getwork
...
class StratumRPCMiningProvider(object):
    ...
    def _send_work(self):
        ...
        self.other.svc_mining.rpc_set_difficulty(starwels_data.target_to_difficulty(x['share_target'])*self.wb.net.DUMB_SCRYPT_DIFF).addErrback(lambda err: None)
    ...
    def rpc_submit(self, worker_name, job_id, extranonce2, ntime, nonce):
        ...
        header = dict(
            ...
            merkle_root=starwels_data.check_merkle_link(starwels_data.hash256(new_packed_gentx), x['merkle_link']),
